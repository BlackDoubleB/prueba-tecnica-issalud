...
export default function PanelFiltros({ dataApi }: { dataApi: Country[] }) {
  ...
  const filtrosCard = useMemo<Country[]>(() => {
    ...
    return dataApi.filter((data) => {

      const filtroPais = nombreNorm ? norm(data.name.common).includes(nombreNorm): true;
      const filtroRegion = regionNorm ? norm(data.region).includes(regionNorm) : true;
      const filtroRango  = data.population >= limiteMin && data.population <= limiteMax;
      
      return filtroPais && filtroRegion && filtroRango;

    });
  }, [dataApi, nombrePais, nombreRegion, min, max]);
      ...
      <div className="flex gap-10">
        ...
        <div className="flex flex-wrap gap-10 flex-1">
          {filtrosCard.length === 0 ? (
            ...
            filtrosCard.map((x) => (
              <Card paises={x} key={x.cca3 ?? x.name.common} />
            ))
          )}
        </div>
      </div>
